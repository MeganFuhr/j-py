...
giphy = os.environ['GIPHY_API']
jid = os.environ['DISCORD_ID']
gid = [int(os.environ['GUILDS_ID'])]
secretWord = os.environ['SECRET_WORD']
mongo = pymongo.MongoClient(os.environ['MONGOURI'], tlsCAFile=ca)
...
async def st(ctx: SlashCommand, options=genericOptions):
    ...
    embed.set_author(name=Help.authorName, url=Help.authorUrl,
                     icon_url=Help.authorIcon_URL)
    embed.add_field(name='!add-insult',
                    value='Add an insult to the database. Anyone except J can add insults.')
    embed.add_field(name='!add-praise',
                    value='Add a praise to the database.  Anyone can add a praise.')
    embed.add_field(name='!add-quote',
                    value='Add a quote to the database.  Only J can add quotes.')
    embed.add_field(
        name='/j', value='Option: gif => Display a random gif of J.')
    embed.add_field(
        name='/j', value='Option: insult => Return a random insult to J.')
    embed.add_field(
        name='/j', value='Option: praise => Return a random praise to J.')
    embed.add_field(
        name='/j', value='Option: quote => Return a random quote of J.')
    embed.add_field(name='/sopranos',
                    value='Display a quote from a favorite J show.')
...
async def on_message(message):
    ...
    if secretWord in message.content:
        embed = discord.Embed(
            ...
            description="You used the SECRET WORD :confetti_ball: :tada:   " +
            secretWord.upper() + "!!!!!!"
        )
        embed.set_image(
            url="https://media.giphy.com/media/NvXLlQvvXSmQM/giphy.gif")
